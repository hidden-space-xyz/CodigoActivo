...
which looks the code up under the `errors.*` namespace of `src/shared/i18n/locales/es.json`.
...
3. **`npm run api:generate`** (Orval) regenerates the typed client; **`npm run api:check`** generates into a
   temporary directory and fails CI if the committed client differs from the contract.
4. **Errors**: a new failure mode adds an `ErrorCode` member in the backend and a Spanish message under
   the frontend's `errors.*` i18n namespace (`src/shared/i18n/locales/es.json`).
