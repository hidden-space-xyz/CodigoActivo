...
- **All user-facing text goes through Vue I18n** — never hardcode string literals in components. Every UI string is a key in `frontend/src/shared/i18n/locales/es.ts`; PrimeVue's own built-ins live in `frontend/src/shared/i18n/primevue-locale.ts`. The app is **Spanish-only** for now (single `es` locale); a second language is a drop-in `en.ts`.
