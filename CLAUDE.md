...
- `frontend/` — Vue 3 + Vite SPA (PrimeVue, TanStack Query). Reads go through OData (`/api/odata`); writes use a client generated from the backend's OpenAPI doc.
...
- **Identity is passed explicitly.** There is no ambient `ICurrentUser`. Controllers read `UserId` (from `CommandControllerBase`, backed by the auth cookie) and pass `Guid userId` into service methods. Auditable entities (`AuditableEntity`) get `CreatedBy`/`CreatedAt`/`UpdatedBy`/`UpdatedAt` set in the service.
...
- **Reads vs writes (CQRS-ish split).** Reads are **OData**: controllers extending `ODataController` under `API/Controllers/OData/`, exposed at `/api/odata` via `API/OData/EdmModelBuilder.cs` (`ODataConventionModelBuilder`, `EnableLowerCamelCase`); services return `IQueryable<XResponse>` projections (member-init, EF-translatable). Writes are `*CommandsController : CommandControllerBase`. `Program.cs` remaps the command controllers' Swagger tags (`TagActionsBy`) and applies `JsonResponseMediaTypeFilter`; a `DeaccentFilterBinder` (custom `IFilterBinder` + a runtime-registered `deaccent()` OData function) powers accent/case-insensitive search. (There is **no** response-caching layer.)
...
- **`src/shared/`** — reusable, app-agnostic base. Segments: `api/` (`generated/` Orval output, `odata.ts`/`odata-models.ts` OData read helpers + hand-declared response types, `http-client.ts` mutator), `ui/` (PrimeVue-based kit, flat components), `lib/` (formatting, richtext, media, `use-crud-feedback`, …), `config/` (`env`, app constants, nav config). Stays free of any upward (`entities`) import.
- **`src/entities/`** — business entities (`event`, `activity`, `announcement`, `resource`, `partner`, `user`, `account`, `session`, `catalog`, `file`, `organization`). Segments: `model/` (domain types, value-objects, global state — e.g. `session` exports the `useSession()` module-level reactive singleton; **no Pinia**), `api/` (request functions that wrap the OData helpers for reads / the generated client for writes + map responses + read vue-query hooks + `query-keys.ts`; mapper is internal), `ui/` (entity cards). The Clean Architecture ceremony (repo interfaces, providers, one-line use-cases) was removed — composables call the `api` functions directly.
...
- **OData reads, generated-client writes.** Reads use OData (`fetchODataList` / `fetchODataEntity` from `shared/api`; OData-only response types are hand-declared in `shared/api/odata-models.ts`; nested collections come inline from backend projections — there is no `$expand`). Writes (and a few non-OData reads like `getApiAuthMe`) use the Orval-generated `useXxx` hooks — configured (`orval.config.ts`) for `client: 'vue-query'` with `httpClient` as the mutator. `http-client.ts` is where cross-cutting HTTP behavior lives: `credentials: 'include'` for the session cookie, lazy fetch + cache of the CSRF token (re-fetched and the request retried once on 400/403), and `ApiError` shaping from ProblemDetails responses. Put shared HTTP concerns in the mutator.
