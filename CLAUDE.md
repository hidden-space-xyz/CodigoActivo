...
- **Public**: home, events, announcements, resources, partners, member registration (adults may register dependent minors), plus the full account lifecycle — login, OTP verification, forgot/reset password, and a self-service account page split into **three tabs** — profile (which also holds the dependent minors), a participation **history** from which members rate finished events, and the participation **certificates** the household has earned, downloadable as PNG or PDF (the active tab round-trips through `?tab=`).
...
- **Email**: admins compose plain-text mail with attachments to one person or to everyone matching the current filters, from both the users table and the *Asistentes* tab (both lists also export to CSV). Two further notifications are **automatic**: signing up for an activity acknowledges the request as pending, and an admin's confirm/reject tells the member the outcome — always to the guardian's address when the enrolled person is a dependent minor. **Every automatic message is delivered by an in-process background queue**, so no user action ever waits on the SMTP relay; admin-written mail stays synchronous because its response reports how many messages were delivered.
...
- **Outbound email is rate-limited *and* queued backend-side, and the frontend is untouched by both.** One always-on guard at the `IEmailSender` boundary covers every *automatic* send and decides **synchronously**, on the request thread; the message is then handed to a bounded in-memory queue drained by one background worker, so the request returns without waiting on SMTP. Admin-written mail is exempt from the guard at any volume and stays synchronous by construction. No API contract changed — a denial (including a full queue) is silent everywhere except `resend-verification`, which reuses the existing `OtpResendCooldownActive`. See [SECURITY.md](SECURITY.md) and `backend/CLAUDE.md`.
- **Participation certificates are derived state, and the sheet is drawn on a `<canvas>`.** There is **no `certificates` table, no migration and no background job that "issues" them when an event ends**: `GET /api/me/certificates` computes the list on every read from the assignments that are already there — one diploma per `(event, member)` with at least one **Confirmed** assignment in an event whose `EventEndsAt` is strictly in the past. That is what makes an admin's late correction (a revoked attendance, a fixed event date) show up immediately instead of leaving an orphan certificate behind, and it is why the feature adds no `ErrorCode`, no `AppStrings` key and no cache tag. The **rendering** is entirely frontend (`features/account/model/certificate-sheet.ts`), like the printable badges and roster — see `frontend/CLAUDE.md` for why it is a canvas rather than DOM/SVG, and why the sheet ignores the theme.
