...
`<Codigoactivo/>` is a nonprofit association's website (León, since 2016). Public side: home, events, announcements, resources, partners, member registration (adults can register dependent minors), plus the full account lifecycle — login, OTP account verification, forgot/reset password, self-service account page (profile, dependent minors, and a participation **history** of past/upcoming events from which members rate finished events). Admin back-office: ten sections (events, event detail/roster/printable badges — the detail has *Actividades*/*Asistentes*/*Opiniones* tabs, announcements, resources, partners, users, catalogs, dashboard with charts). Admins can also **write a plain-text email with attachments** to one person or to everyone matching the current filters, from both the users table and the *Asistentes* tab; both lists export to CSV. On top of that, two **automatic** notifications go out with no one pressing "send": signing up for an activity (for yourself or for a minor in your care) acknowledges the request as pending, and an admin confirming or rejecting it tells the member the outcome — always to the guardian's address when the enrolled person is a dependent minor.
...
- **Keep the checks green — but nothing enforces them for you.** There is no CI (`.github/workflows` is empty and untracked) and no git hooks, so every gate below is manual:
  - Backend: analyzers (Meziantou + Sonar + the SDK's CA/IDE rules) report **warnings, never errors** — `dotnet build` succeeds with violations present, by design and by explicit configuration. The whole analyzer setup lives in three files that are **byte-identical to their counterparts in the BackupZCrypt repository** — `backend/Directory.Build.Analyzers.props`, `backend/Directory.Build.targets`, `backend/SonarLint.xml` — plus lines 3–176 of `backend/.editorconfig`. Edit one repo without the other and the two drift; `backend/CLAUDE.md` carries the `diff` command that proves they still match. **None of those files carries comments** — every rationale lives in the Markdown docs.
  - **Warnings are re-reported on every build, including an up-to-date one.** MSBuild normally skips `CoreCompile` when nothing changed, so the analyzers never run and a warm `dotnet build` prints `0 Warning(s)` with hundreds of live violations. `backend/Directory.Build.targets` defeats that with a sentinel output file that is never created. Every build therefore pays for a full compile+analysis pass; opt out for one invocation with `-p:AlwaysReportAnalyzerWarnings=false`.
...
- **Outbound email is rate-limited backend-side**, and the frontend is untouched by it. One guard at the `IEmailSender` boundary (`backend/src/CodigoActivo.Infrastructure/Communication/ThrottledEmailSender.cs`) covers every *automatic* send — verification, password reset, activity notifications — with a per-destination-address budget plus a process-wide one. **Admin-written email is exempt at any volume**: it is the only consumer of the separate `IEmailTransport`, which is what makes the exemption a compile-time property rather than a convention. No API contract changed — a denial is silent everywhere except `resend-verification`, which reuses the existing `OtpResendCooldownActive`. **The guard is always on** — only its numbers are configurable (`EmailGuard:*` in `appsettings.json`), and each falls back to its default on a bad value. See [SECURITY.md](SECURITY.md) and `backend/CLAUDE.md`.
