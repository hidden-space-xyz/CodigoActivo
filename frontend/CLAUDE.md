...
Slices are **kebab-case** and each exposes a public API via `index.ts`. `fsd/insignificant-slice` is deliberately off in `steiger.config.ts` (which carries no comments) to allow single-consumer slices: it is the most commonly disabled FSD rule because it penalizes preparatory slicing, and enabling it would dissolve single-consumer slices into their consumer and leave the parallel `manage-*` admin features structurally inconsistent. That config also ignores `**/shared/api/generated/**`, which is Orval output and does not follow FSD slicing.
...
- `generate-icons.py` carries no comments; its three non-obvious choices are: the favicon sizes let the mark fill the whole canvas with no margin, because at 16 px a safe margin costs legibility; `apple-touch-icon.png` keeps a margin instead, since iOS applies its own rounded mask; and the bundled copies are palette-quantized to 256 colours, which the gradients survive (mean composite error ≈1.3/255 against both theme backgrounds) at a fraction of the bytes. The 640 px wide variant exists to cover the ~330 CSS px hero at 2× DPR.
...
`frontend/docker/` holds three files — `default.conf`, `security-headers.conf`, `proxy-api.conf` — and **none of them carries comments**, so every non-obvious behaviour is recorded here. Each item below is a trap that will silently break something if it is "cleaned up".
...
- **Real client IP comes from `X-Forwarded-For`, but only when the TCP peer is private.** The documented topology puts a TLS-terminating proxy in front, so without this every client would share one rate-limit bucket. **Caveat:** if the container is ever exposed *directly* to the internet, Docker's NAT still presents a private peer, so clients could spoof the header to dodge the limits — in that topology remove the `realip` directives or scope `set_real_ip_from` to the actual proxy address.
