...
- `src/shared/api/http-client.ts` is the Orval mutator: native `fetch`, `credentials: 'include'` (cookie session), prefixes `VITE_API_BASE_URL`. It **transparently handles CSRF**: fetches `/api/auth/csrf` before unsafe methods, sends `X-CSRF-TOKEN`, retries once on an invalid-token error. Call `resetCsrfToken()` after login/logout (the session entity's requests already do). Failures throw `ApiError` (`status`, `code`, `traceId`); user-facing copy comes from `src/shared/api/error-messages.ts`.
