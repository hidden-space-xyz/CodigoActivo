...
const props = defineProps<{
  ...
  visible: boolean
  /** User who would receive the admin role; only their name is shown. */
  user: User | null
  /** Shows a loading state on the confirm button while the parent sends the grant. */
  saving: boolean
...
const emit = defineEmits<{
  ...
  /** Fired with the signed-in admin's password, never empty, to authorize the grant. */
...
<template>
  <el-dialog
    :model-value="visible"
    :title="$t('features.manageUsers.grantAdmin.header')"
  ...
  >
    <form class="form" @submit.prevent="confirm">
      <p class="form__message">
        {{ $t('features.manageUsers.grantAdmin.message', { fullName: targetName }) }}
      </p>
      <div class="form__field">
        <label for="grant-admin-password">{{
          $t('features.manageUsers.grantAdmin.passwordLabel')
        }}</label>
        <el-input
          id="grant-admin-password"
          v-model="password"
          type="password"
          show-password
        ...
        <small v-if="missing" class="form__error">{{
          $t('features.manageUsers.grantAdmin.passwordRequired')
        }}</small>
    ...
    <template #footer>
      ...
      <Button
        :label="$t('features.manageUsers.grantAdmin.confirm')"
        type="primary"
        :loading="saving"
        @click="confirm"
      />
