...
Options are plain objects built from config and registered as singletons — **not `IOptions<T>`** — and they never live in Domain: `Application/Options/` holds the ones Application or API consumes (`ApplicationOptions`, `AccountVerificationOptions`, `PasswordResetOptions`, `ManualEmailOptions`, `FileUploadOptions`); `Infrastructure/Communication|Storage/` holds the rest (`SmtpOptions`, `EmailGuardOptions`, `EmailQueueOptions`, `FileStorageOptions`). `Architecture/DataShapeTests.cs` fails if an `*Options` type reappears in Domain.

## Project structure (dependency rules)

```
Domain          entities, repository interfaces, Result/Error — depends on nothing
Application     command/query handlers (business logic), DTOs, mapping — depends on Domain only
...
| `S1200` | 9 | Class coupling: the composition root (`DependencyInjection`), `Projections`, `DemoDataSeeder`, the four biggest controllers (`Activities`/`Auth`/`Events`/`Users` — inherent to injecting one concrete handler per action) and the two largest handlers (`AssignHouseholdCommandHandler`, `GetDashboardAnalyticsQueryHandler`). `SonarLint.xml` cannot change (byte-identical with BackupZCrypt); "fixing" these means breaking verbatim use-case bodies or the per-action injection pattern. The services' `S107` entries died with the services — the only survivor is one 12-parameter test-data builder. |
...
- Handlers return `Task<Result<TResponse>>` (or `Task<Result>` for body-less mutations). `Domain/Common/Result.cs` has implicit conversions: success is `return dto;`, failure is `return Error.NotFound(ErrorCode.UserNotFound);`.
...
- **Handlers** (`Application/<Aggregate>/Commands|Queries/`, one use case per file): a sealed positional record message (verb-first: `CreateEventCommand`, `ListEventsQuery`) colocated with its sealed `<Message>Handler` implementing `ICommandHandler<,>`/`IQueryHandler<,>` (`Abstractions/Messaging/IMessaging.cs`). Primary-constructor DI on repository interfaces — plus `IUnitOfWork`/`IClock` for mutations, `IQueryExecutor` for paged reads, `IPasswordHasher` where needed. **Never inject `DbContext`.** Binding DTOs (`*ListQuery`, `*Request`) never implement the message interfaces — the controller wraps them (plus `UserId`/`IsAdmin` from claims) into the message; cached list reads keep byte-identical keys by serializing the inner DTO (`CacheKeys.For("events:list", query.Filters)`). A command handler may inject a concrete query handler of its own aggregate for read-your-write responses, never the reverse. `UnitTests/Architecture/HandlerConventionTests.cs` enforces naming, namespaces, one-handler-per-message and that query handlers never depend on `IUnitOfWork`, `ICacheInvalidator` or an email port; `Composition/HandlerRegistrationTests.cs` fails on an unregistered handler.
- **Collaborators** (aggregate-root folders): logic shared by several handlers of an aggregate — `SignupGate`, `ActivityValidator`, `ActivitySignupNotifier` and static `SignupPolicy` (`Activities/`), `AccountEmails` and `OtpValidator` (`Auth/`), `EventCategoryChecker` and static `EventRules` (`Events/`), `FileUploadValidator` and static `FileNaming` (`Files/FileRules.cs`), `IOrphanFileCleaner`/`OrphanFileCleaner` (`Files/`, the one collaborator with an interface: it is the cross-aggregate seam five aggregates substitute in tests), `ManualEmailDispatcher` (`Emails/`). All registered explicitly in `Composition/DependencyInjection.cs`; in unit tests they are constructed real over the same substitutes — only `IOrphanFileCleaner` is substituted.
...
- Clients never cache, with two deliberate exceptions: `CacheControlMiddleware` stamps `Cache-Control: no-store` on every `/api` response without an explicit header; `files/{id}/content` sends `private, no-cache` + ETag instead (always revalidates, cheap 304s) and `UpdateFileCommandHandler` bumps `UploadedAt` so the ETag rotates on content replacement. The SEO endpoints are the other exception (`public, max-age=1h`).
...
**All four flows are delivered off the request thread.** Every `IEmailSender.SendAsync` call now ends at `ChannelEmailDispatcher` (see The background dispatcher), so the multi-second SMTP round trip no longer sits between the user's click and their toast. Only the admin flow (#4) still talks to `IEmailTransport` directly — through `ManualEmailDispatcher` — because its response body reports delivered/failed counts. Two consequences to keep in mind when reading the code: an OTP or reset code is persisted after the message is **queued**, not after it is delivered, and a relay failure is logged by the worker rather than by the handler that composed the message. The `try`/`catch` blocks in the auth handlers and in `ActivitySignupNotifier` are still correct and still needed — they wrap the message-building work too, which for the activity notifications is three DB queries — but their `catch (Exception)` arms no longer see SMTP failures, so the `OtpLastSentAt = null` compensation in `RegisterCommandHandler`'s send helper is effectively vestigial. It is kept because the handler must not assume its `IEmailSender` is queued; the practical cost of losing it is that a member whose verification mail fails at the relay waits out the 60 s resend cooldown instead of retrying immediately.

3. **Activity-signup notifications** — the only *automatic* flow; nobody triggers it explicitly, it rides along three assignment commands through `ActivitySignupNotifier`. `AssignActivityCommandHandler` and `AssignHouseholdCommandHandler` send "we got your request, it is pending" (`ActivitySignupEmail.cs`); `ChangeAssignmentStatusCommandHandler` sends the admin's verdict (`ActivitySignupDecisionEmail.cs`) — **only** when the status actually changes *and* the new one is `Confirmed`/`Denied`, so re-applying a status or moving a row back to `Solicitada` stays silent. Both share `ActivityEmailDetails.cs` for the activity/event/schedule block; dates are rendered `dd/MM/yyyy HH:mm` on purpose, because the prod image runs with `DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=true` and any culture-aware month name would come out in English. Two rules the code must keep: **the recipient is the account that owns the address** — a dependent minor has none, so `ResolveRecipient` falls back to the guardian and the copy names the child instead of saying "your signup"; and **delivery never fails the write** — the assignment is already committed when the send happens, so every path is wrapped in a `try`/`catch` that logs and swallows, exactly like `ForgotPasswordCommandHandler`'s password-reset send.
4. **Manual (admin-written) mail** — the only `[AllowOnlyAdmin]` flow: `POST /api/emails/users/{userId}`, `POST /api/emails/users` (`[FromQuery] UserListQuery`), `POST /api/emails/events/{eventId}/attendees` (`[FromQuery] EventAttendeeListQuery`) → `EmailsController` → the three `SendEmailTo*` command handlers → `ManualEmailDispatcher` → `ManualEmail.cs`. These are the repo's only `[FromForm]` actions: `multipart/form-data` carrying `subject`, `body` and repeated `attachments` parts, with the list filters still on the query string so the admin's on-screen filters map 1:1. Nothing is persisted: no `SaveChangesAsync`, no cache invalidation, no file rows.
...
- **Recipients are resolved server-side** through the shared `Querying/UserFilters.cs` predicates — the same ones `ListUsersQueryHandler` and `ListEventAttendeesQueryHandler` use, so change them there, never by copying — and materialised with `IQueryExecutor.ToListAsync`, because `PageQuery` would clamp a paged read to 100. Users without an address (dependent minors) are skipped and counted in `SendEmailResultResponse.Skipped`.
...
`GET /api/me/certificates` (`MeController` → `GetEventCertificatesQueryHandler`) returns the caller's household's diplomas. **Nothing is persisted** — see the root `CLAUDE.md`. Five details are load-bearing:
...
- xUnit **v3**, **AwesomeAssertions** (FluentAssertions fork, same `.Should()` API), NSubstitute. Unit tests mirror the src namespace tree: one test file per handler under `UnitTests/Application/<Aggregate>/Commands|Queries/` (`<Handler>Tests`, method segment always `HandleAsync` — the class name carries the use case), shared builders in an `internal static <Aggregate>TestData` per aggregate (static factories + extensions over the repository substitutes, consumed with `using static`), collaborators constructed real over those substitutes. Hand-rolled fakes in `UnitTests/TestSupport/` (`FakePasswordHasher`, `FakeQueryExecutor`, `FakeHybridCache`, `TestClock`, `FixedTimeProvider`, `RecordingEmailSender`, `RecordingEmailDispatcher`, `RepositoryStubs`). `UnitTests/Architecture/` holds the reflection-based architecture suite (layer references, handler conventions, data-shape taxonomy) and `Composition/` the DI wiring tests — both fail on convention drift, not just on behavior.
...
- **Type colocation is intentional**: all repository interfaces in one file, each use case's `Command`/`Query` record colocated with its `Handler`, request+response DTOs per aggregate in one `*Dtos.cs`.
- **Data-carrier taxonomy** (see `ARCHITECTURE.md` for the full table): `Application/DTOs/` is wire-only (`*Request`/`*Response` records, no `Stream` properties); options never live in Domain; projection rows are `private sealed record` with `{ get; init; }` nested in their handler (member-init only — a positional ctor stops EF translating); a type used by exactly one class is a private nested type.
