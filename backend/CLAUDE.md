...
- `SMTP_*` for MailKit.
- `FILE_STORAGE_ROOT` (default `files`, relative to the content root) — where `LocalFileSystemRepository` writes uploads; Compose points it at `/app/files`, backed by a named volume. The size cap is not an env var: it is the `appsettings` `FileStorage:MaxSizeBytes` knob.
